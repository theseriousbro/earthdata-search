...
describe "Spatial and temporal extraction", reset: false do
  ...
  context "extracted spatial information" do
    ...
    it "is set in the query" do
      ...
      expect(Project.find(project_id).path).to eql('/search/collections?m=31.168933999999993!-100.0768425!0!1!0!0%2C2&ok=Texas&sb=-106.645646%2C25.837163999999998%2C-93.508039%2C36.500704')
    end
  ...
  context "extracted spatial and temporal information" do
    ...
    it 'is set in the query and adds q= and ok= query params to the url' do
      ...
      expect(path).to eql('/search/collections?m=42.314350000000005!-70.96997850000001!0!1!0!0%2C2&qt=2015-12-01T00%3A00%3A00.000Z%2C2016-03-31T23%3A59%3A59.000Z&q=snow+cover&ok=snow+cover+in+Boston+last+winter&sb=-71.191155%2C42.22788%2C-70.748802%2C42.40082')
